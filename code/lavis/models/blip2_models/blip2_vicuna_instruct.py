...
class Blip2VicunaInstruct(Blip2Base):
    ...
    def generate(
        ...
        if image.dim() == 5:
            ...
            ## cd_comments: get negative cd image embeds for contrastive decoding
            if images_cd_negative is not None:
                with self.maybe_autocast():
                    image_embeds_cd_negative = self.ln_vision(self.visual_encoder(images_cd_negative))
                image_atts_cd_negative = torch.ones(image_embeds_cd_negative.size()[:-1], dtype=torch.long).to(image.device)
            ...
            if self.qformer_text_input:
                ...
                ## cd_comments: input Qformer for positive contrastive
                if images_cd is not None:
                ...
                ## cd_comments: input Qformer for negative contrastive
                if images_cd_negative is not None:
                    query_output_cd_negative = self.Qformer.bert(
                        text_Qformer.input_ids,
                        attention_mask=Qformer_atts,
                        query_embeds=query_tokens,
                        encoder_hidden_states=image_embeds_cd_negative,
                        encoder_attention_mask=image_atts_cd_negative,
                        return_dict=True,
                    )
            else:
                query_output = self.Qformer.bert(
            ...
            ## cd_comments: for generate with positive contrastive
            if images_cd is not None:
            ...
            ## cd_comments: for generate with negative contrastive
            if images_cd_negative is not None:
                inputs_llm_cd_negative = self.llm_proj(query_output_cd_negative.last_hidden_state[:,:query_tokens.size(1),:])
                atts_llm_cd_negative = torch.ones(inputs_llm_cd_negative.size()[:-1], dtype=torch.long).to(image.device)
        ...
        with self.maybe_autocast():
            ...
            ## cd_comments: format for positive contrastive
            if images_cd is not None:
            ...
            ## cd_comments: format for negative contrastive
            if images_cd_negative is not None:
                inputs_embeds_cd_negative = self.llm_model.get_input_embeddings()(llm_tokens.input_ids)
                inputs_embeds_cd_negative = torch.cat([inputs_llm_cd_negative, inputs_embeds_cd_negative], dim=1)
                attention_mask_cd_negative = torch.cat([atts_llm_cd_negative, llm_tokens.attention_mask], dim=1)
            ...
            outputs = self.llm_model.generate(
                ...
                images_cd_negative=inputs_embeds_cd_negative if images_cd_negative is not None else None,
                cd_beta=cd_beta if cd_beta is not None else None,
                cd_alpha=cd_alpha if cd_alpha is not None else None,
                cd_gamma=cd_gamma if cd_gamma is not None else None
            )
